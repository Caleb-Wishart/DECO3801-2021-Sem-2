...
from .models import ChannelVisibility, Grade, ResourceDifficulty, Subject
...
def add_pseudo_comment_to_resource(rid: int, accessors_id: list):
    ...
    for replier in repliers:
        if (
            reply_to_resource_comment(uid=replier, resource_comment_id=comment_id, reply=choice(random_texts))
...
def add_pseudo_channel_post_and_replies(cid: int, accessors_id: list):
    ...
    channel_post = post_on_channel(
        uid=poster_uid,
        title=choice(random_texts),
        cid=cid,
        text=" ".join(sample(random_texts, k=random.randint(1, 10))),
    )
    ...
    repliers = sample(accessors_id, min(2, len(accessors_id)))
    for replier in repliers:
        post_reply = comment_on_channel_post(
            uid=replier, post_id=channel_post, text=" ".join(sample(random_texts, k=random.randint(1, 10)))
        )
        for voter in get_random_voters(accessors_id):
            vote_channel_post_comment(uid=voter, post_comment_id=post_reply, upvote=bool(random.getrandbits(1)))
...
# single pdf file
src0 = add_resource(
    title="CS50 at Harvard - The Most Rewarding Class I Have Taken" " . . . Ever!",
    resource_link="resource/CS50_at_Harvard_The_Most_Rewarding" "_Class_I_Have_Taken_Ever.pdf",
...
"""
src1 = add_resource(
    title="Grade 2 Math 1.1, Understanding Addition",
...
"""
src2 = add_resource(
    title="3rd Grade Math 8.6, Relate Fractions and Whole Numbers",
...
"""
src3 = add_resource(
    title="Drama Lesson Activities, Grade 4-6: Creative Play",
    resource_link="https://www.youtube.com/embed/u8VEuS-32JM",
    difficulty=ResourceDifficulty.EASY,
    creaters_id=sample(users_id, k=1),
    tags_id=[tags["Drama_activities"]],
    grade=Grade.YEAR_6,
...
"""
src5 = add_resource(
    title="4th Grade Math Input-Output Tables",
...
resource_titles = [
    ...
    "Grade 12 Calculus An Introduction to Calculus and Overview" " of Key Concepts NSC DBE Maths NTE",
    "Factoring Quadratic - Expressions (B)",
]
resource_links = [
    "Learn_Chinese_for_Beginners_Beginner_Chinese_Lesson_1_Self-" "Introduction_in_Chinese_Mandarin_1.1.mp4",
    "GRADE_12_TRIGONOMETRY_WORKSHEET.zip",
    "ANALYTICAL_GEOMETRY_-_The_basics_(a_compilation).mp4",
    "Grade_12_Calculus_An_Introduction_to_Calculus_and_Overview_" "of_Key_Concepts_NSC_DBE_Maths_NTE.mp4",
...
subjects = [Subject.CHINESE, Subject.MATHS_C, Subject.MATHS_C, Subject.MATHS_C, Subject.MATHS_C]
resource_difficulty = [
    ResourceDifficulty.EASY,
...
descriptions = [
    "Learn Chinese w/ ChineseFor.Us Beginner Chinese Lesson 1: Self-introduction"
...
resource_tags = [
    [tags["1_min_tutorial"], tags["distance_teaching"]],
...
for i in range(6, 11):
    ...
    globals()[f"src{i}"] = add_resource(
        title=resource_titles[i - OFFSET],
    ...
    for uid in voters:
        vote_resource(uid=uid, rid=globals()[f"src{i}"], upvote=bool(random.getrandbits(1)))

# create 5 channels
channel_names = sample(random_texts, k=5)
visibility = 2 * [ChannelVisibility.PUBLIC] + [ChannelVisibility.INVITE_ONLY] + 2 * [ChannelVisibility.FULLY_PRIVATE]
...
for i in range(0, 5):
    if visibility[i] != ChannelVisibility.PUBLIC:
        ...
        personnel = None
    globals()[f"cid{i}"] = create_channel(
    ...
    for j in range(random.randint(0, max_cont)):
        add_pseudo_channel_post_and_replies(
            cid=globals()[f"cid{i}"], accessors_id=personnel if personnel is not None else users_id
        )
